...
function getResult(matrixProp, curProp, clearLines, animateColor) {
  const shape = curProp && curProp.shape;
  const xy = curProp && curProp.xy;
...
function Matrix({ matrix, cur, reset }) {
  ...
  function over(props) {
    let newOverState = getResult(props.matrix, props.cur, false, animateColor);
  ...
  React.useEffect(() => {
    ...
    if (!clears && overs && !isOver) {
      over({ matrix, cur, reset });
    }
  }, [matrix, reset, cur, clearLines, isOver]);
