...
function App({
  ...
  React.useEffect(() => {
    ...
    if (lastRecord) { // 读取记录
      if (lastRecord.cur && !lastRecord.pause) { // 拿到上一次游戏的状态, 如果在游戏中且没有暂停, 游戏继续
        const currentSpeedRun = speedRun;
        let timeout = speeds[currentSpeedRun - 1] / 2; // 继续时, 给予当前下落速度一半的停留时间
        // 停留时间不小于最快速的速度
        timeout = currentSpeedRun < speeds[speeds.length - 1]
  ...
  const size = (() => {
    ...
    if (ratio < 1.5) {
      ...
      scale = currentW / 640;
      filling = (currentH - (960 * scale)) / scale / 3;
      css = {
