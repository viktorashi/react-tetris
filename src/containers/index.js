...
function App({
  speedRun, drop, matrix, cur, reset, points, max, clearLines, startLines,
  speedStart, next, music, pause, keyboard,
}) {
  ...
  React.useEffect(() => {
    ...
    if (lastRecord) { // 读取记录
      if (lastRecord.cur && !lastRecord.pause) {
        ...
        const recordSpeedRun = speedRun;
        // 停留时间不小于最快速的速度
        const timeout = recordSpeedRun < speeds[speeds.length - 1]
